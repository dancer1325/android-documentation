...
**Source:** [https://developer.android.com/tools](https://developer.android.com/tools)
...
We recommend setting the environment variable for [`ANDROID_HOME`](/studio/command-line/variables#envar) when using the command line. Also, set your command search path to include `ANDROID_HOME/tools`, `ANDROID_HOME/tools/bin`, and `ANDROID_HOME/platform-tools` to find the most common tools. The steps vary depending on your OS, but read [How to set environment variables](/studio/command-line/variables#set) for general guidance. 
...
See [SDK Build Tools release notes](/studio/releases/build-tools) for more information. 
...
See [SDK Platform Tools release notes](/studio/releases/platform-tools) for more information. 
...
In addition to downloading from the SDK Manager, you can download the SDK Platform Tools [here](/studio/releases/platform-tools#downloads.html).
...
See [Android Emulator release notes](/studio/releases/emulator) for more information. 
...
[Jetifier](/studio/command-line/jetifier) reads a library that uses Support Library classes and outputs an equivalent library that uses the newer AndroidX classes.
