...
**Source:** [https://developer.android.com/topic/performance/background-optimization](https://developer.android.com/topic/performance/background-optimization)
...
On this page, we will learn how to use alternative methods, such as `[JobScheduler](/reference/android/app/job/JobScheduler)`, to adapt your app to these new restrictions. 

## User-initiated restrictions

On the [**Battery usage** page within system settings](https://support.google.com/pixelphone/answer/7015477), the user can choose from the following options:
...
The specific restrictions are listed in [ Power management restrictions](/topic/performance/power/power-details). 

##  Restrictions on receiving network activity broadcasts 

Apps targeting Android 7.0 (API level 24) do not receive `[CONNECTIVITY_ACTION](/reference/android/net/ConnectivityManager#CONNECTIVITY_ACTION)` broadcasts if they register to receive them in their manifest, and processes that depend on this broadcast will not start. This could pose a problem for apps that want to listen for network changes or perform bulk network activities when the device connects to an unmetered network. Several solutions to get around this restriction already exist in the Android framework, but choosing the right one depends on what you want your app to accomplish. 

**Note:** A `[BroadcastReceiver](/reference/android/content/BroadcastReceiver)` registered with `[Context.registerReceiver()](/reference/android/content/Context#registerReceiver\(android.content.BroadcastReceiver, android.content.IntentFilter\))` continues to receive these broadcasts while the app is running. 

###  Schedule network jobs on unmetered connections 

When using the `[JobInfo.Builder](/reference/android/app/job/JobInfo.Builder)` class to build your `[JobInfo](/reference/android/app/job/JobInfo)` object, apply the `[setRequiredNetworkType()](/reference/android/app/job/JobInfo.Builder#setRequiredNetworkType\(int\))` method and pass `[JobInfo.NETWORK_TYPE_UNMETERED](/reference/android/app/job/JobInfo)` as a job parameter. The following code sample schedules a service to run when the device connects to an unmetered network and is charging: 
...
When the conditions for your job are met, your app receives a callback to run the `[onStartJob()](/reference/android/app/job/JobService#onStartJob\(android.app.job.JobParameters\))` method in the specified `JobService.class`. To see more examples of `[JobScheduler](/reference/android/app/job/JobScheduler)` implementation, see the [JobScheduler sample app](https://github.com/googlearchive/android-JobScheduler/tree/master/kotlinApp#readme). 

A new alternative to JobScheduler is WorkManager, an API that allows you to schedule background tasks that need guaranteed completion, regardless of whether the app process is around or not. WorkManager chooses the appropriate way to run the work (either directly on a thread in your app process as well as using JobScheduler, FirebaseJobDispatcher, or AlarmManager) based on such factors as the device API level. Additionally, WorkManager does not require Play services and provides several advanced features, such as chaining tasks together or checking a task's status. To learn more, see [WorkManager](/arch/work). 

###  Monitor network connectivity while the app is running 

Apps that are running can still listen for `CONNECTIVITY_CHANGE` with a registered `[BroadcastReceiver](/reference/android/content/BroadcastReceiver)`. However, the `[ConnectivityManager](/reference/android/net/ConnectivityManager)` API provides a more robust method to request a callback only when specified network conditions are met. 

`[NetworkRequest](/reference/android/net/NetworkRequest)` objects define the parameters of the network callback in terms of `[NetworkCapabilities](/reference/android/net/NetworkCapabilities)`. You create `[NetworkRequest](/reference/android/net/NetworkRequest)` objects with the `[NetworkRequest.Builder](/reference/android/net/NetworkRequest.Builder)` class. `[registerNetworkCallback()](/reference/android/net/ConnectivityManager#registerNetworkCallback\(android.net.NetworkRequest, android.net.ConnectivityManager.NetworkCallback\))` then passes the `[NetworkRequest](/reference/android/net/NetworkRequest)` object to the system. When the network conditions are met, the app receives a callback to execute the `[onAvailable()](/reference/android/net/ConnectivityManager.NetworkCallback#onAvailable\(android.net.Network\))` method defined in its `[ConnectivityManager.NetworkCallback](/reference/android/net/ConnectivityManager.NetworkCallback)` class. 

The app continues to receive callbacks until either the app exits or it calls `[unregisterNetworkCallback()](/reference/android/net/ConnectivityManager#unregisterNetworkCallback\(android.app.PendingIntent\))`. 

##  Restrictions on receiving image and video broadcasts 

In Android 7.0 (API level 24), apps are not able to send or receive `[ACTION_NEW_PICTURE](/reference/android/hardware/Camera#ACTION_NEW_PICTURE)` or `[ACTION_NEW_VIDEO](/reference/android/hardware/Camera#ACTION_NEW_VIDEO)` broadcasts. This restriction helps alleviate the performance and user experience impacts when several apps must wake up in order to process a new image or video. Android 7.0 (API level 24) extends `[JobInfo](/reference/android/app/job/JobInfo)` and `[JobParameters](/reference/android/app/job/JobParameters)` to provide an alternative solution. 

###  Trigger jobs on content URI changes 

To trigger jobs on content URI changes, Android 7.0 (API level 24) extends the `[JobInfo](/reference/android/app/job/JobInfo)` API with the following methods: 

`JobInfo.TriggerContentUri()`
     Encapsulates parameters required to trigger a job on content URI changes. 
`JobInfo.Builder.addTriggerContentUri()`
     Passes a `TriggerContentUri` object to `[JobInfo](/reference/android/app/job/JobInfo)`. A `[ContentObserver](/reference/android/database/ContentObserver)` monitors the encapsulated content URI. If there are multiple `TriggerContentUri` objects associated with a job, the system provides a callback even if it reports a change in only one of the content URIs. 
     Add the `TriggerContentUri.FLAG_NOTIFY_FOR_DESCENDANTS` flag to trigger the job if any descendants of the given URI change. This flag corresponds to the `notifyForDescendants` parameter passed to `[registerContentObserver()](/reference/android/content/ContentResolver#registerContentObserver\(android.net.Uri, boolean, android.database.ContentObserver\))`. 

**Note:** `TriggerContentUri()` cannot be used in combination with `[setPeriodic()](/reference/android/app/job/JobInfo.Builder#setPeriodic\(long\))` or `[setPersisted()](/reference/android/app/job/JobInfo.Builder#setPersisted\(boolean\))`. To continually monitor for content changes, schedule a new `[JobInfo](/reference/android/app/job/JobInfo)` before the app’s `[JobService](/reference/android/app/job/JobService)` finishes handling the most recent callback. 
...
When the system reports a change in the specified content URI(s), your app receives a callback and a `[JobParameters](/reference/android/app/job/JobParameters)` object is passed to the `[onStartJob()](/reference/android/app/job/JobService#onStartJob\(android.app.job.JobParameters\))` method in `MediaContentJob.class`. 

###  Determine which content authorities triggered a job 

Android 7.0 (API level 24) also extends `[JobParameters](/reference/android/app/job/JobParameters)` to allow your app to receive useful information about what content authorities and URIs triggered the job: 
...
The following sample code overrides the `[JobService.onStartJob()](/reference/android/app/job/JobService#onStartJob\(android.app.job.JobParameters\))` method and records the content authorities and URIs that have triggered the job: 
...
The following [Android Debug Bridge (ADB)](/tools/help/adb) commands can help you test app behavior with background processes disabled: 
