...
**Source:** [https://developer.android.com/topic/performance/sqlite-performance-best-practices](https://developer.android.com/topic/performance/sqlite-performance-best-practices)
...
[Enable WAL](/reference/android/database/sqlite/SQLiteDatabase#enableWriteAheadLogging\(\)) unless you are using [`ATTACH DATABASE`](https://www.sqlite.org/lang_attach.html).
...
For tables that have a primary key other than `INTEGER` or a composite of columns, consider [`WITHOUT ROWID`](https://www.sqlite.org/withoutrowid.html).
...
The previous example requires a full table scan without an index to find all customers from Paris. This is called _linear complexity_. SQLite needs to read all the rows and only keep the rows that match customers from Paris. To fix this, you can add an index:
