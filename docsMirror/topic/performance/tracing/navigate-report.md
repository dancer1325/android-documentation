...
**Source:** [https://developer.android.com/topic/performance/tracing/navigate-report](https://developer.android.com/topic/performance/tracing/navigate-report)
...
![Screenshot of Systrace report](/static/topic/performance/images/tracing/cpu-activity-collapsed.svg) **Figure 1.** Sample CPU activity (collapsed view) in a Systrace report 

* * *

![Screenshot of Systrace report](/static/topic/performance/images/tracing/cpu-activity-expanded.svg) **Figure 2.** Sample CPU activity (expanded view) that shows CPU clock frequency in a Systrace report 
...
![Screenshot of Systrace report](/static/topic/performance/images/tracing/surface-flinger.svg) **Figure 3.** Sample Surface Flinger graph in a Systrace report 

### Display frames

This section, often the tallest in the report, depicts a multicolored line followed by stacks of bars. These shapes represent the status and frame stack of a particular thread that's been created. Each level of the stack represents a call to `beginSection()`, or the beginning of a [custom trace event](/topic/performance/tracing/custom-events) that you've defined for your app or game.
...
**1** | Change the currently-active selection model to the "select" mode. Corresponds to the 1st button that appears in the mouse selector toolbar (see image to the right). |  ![Screenshot of mouse selector toolbar in Systrace report](/static/topic/performance/images/tracing/mouse-selection-toolbar-1.svg)  
**2** | Change the currently-active selection model to the "pan" mode. Corresponds to the 2nd button that appears in the mouse selector toolbar (see image to the right). |  ![Screenshot of mouse selector toolbar in Systrace report](/static/topic/performance/images/tracing/mouse-selection-toolbar-2.svg)  
**3** | Change the currently-active selection model to the "zoom" mode. Corresponds to the 3rd button that appears in the mouse selector toolbar (see image to the right). |  ![Screenshot of mouse selector toolbar in Systrace report](/static/topic/performance/images/tracing/mouse-selection-toolbar-3.svg)  
**4** | Change the currently-active selection model to the "timing" mode. Corresponds to the 4th button that appears in the mouse selector toolbar (see image to the right). |  ![Screenshot of mouse selector toolbar in Systrace report](/static/topic/performance/images/tracing/mouse-selection-toolbar-4.svg)  
...
![Problematic
  frame selected](/static/topic/performance/images/tracing/frame-selected.png) **Figure 5.** Selecting the problematic frame, an alert appears below the trace report identifying the problem 

After you select a slow frame, you may see an alert in the bottom pane of the report. The alert shown in Figure 5 calls out that the primary problem with the frame is that too much time is spent inside [`ListView`](/reference/android/widget/ListView) recycling and rebinding. There are links to the relevant events in the trace that explain more about what the system is doing during this time.
...
![Alert tab shown](/static/topic/performance/images/tracing/frame-selected-alert-tab.png) **Figure 6.** Clicking the **Alert** button reveals the alert tab 
