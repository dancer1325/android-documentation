# Excessive Mobile Network Usage in Background  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/vitals/bg-network-usage](https://developer.android.com/topic/performance/vitals/bg-network-usage)
...
Android vitals considers background network usage excessive when an app is sending and receiving a combined total of 50 MB per day while running in the background. In Play Console you can check percentage of _battery sessions_ that exhibit this behavior.

The definition of _battery session_ depends on the platform version.
...
Tools such as Battery Historian can help you gain more insight into your app's mobile-network usage. Battery Historian provides a visualization of mobile-radio use on a per-app basis, which can help you gain a clearer picture of what's happening with your app. For more information about Battery Historian, see [Analyzing Power Use with Battery Historian](/topic/performance/power/battery-historian#asd). In investigating your app's mobile-network-usage behavior, you should take particular note of the _Mobile network use_ line.

For information about the mechanics of using Battery Historian, see [Batterystats and Battery Historian Walkthrough](/topic/performance/power/setup-battery-historian).

## Reduce mobile network usage

You can move your app’s mobile-network usage to the foreground, alerting the user to the fact that a download is in progress, and providing them with controls to pause or stop the download. To do so, call [`DownloadManager`](/reference/android/app/DownloadManager), and set [`setNotificationVisibility(int)`](/reference/android/app/DownloadManager.Request#setNotificationVisibility\(int\)) as appropriate.
...
[ Next Excessive Battery Usage  arrow_forward  ](/topic/performance/vitals/excessive-battery-usage)
