# Record Java/Kotlin allocations  |  Android Studio  |  Android Developers

**Source:** [https://developer.android.com/studio/profile/record-java-kotlin-allocations](https://developer.android.com/studio/profile/record-java-kotlin-allocations)
...
To record Java/Kotlin allocations, [select the **Track Memory Consumption (Java/Kotlin Allocations)** task](/studio/profile#start-profiling) from the profiler **Home** tab. Note that you need a [debuggable app](/studio/profile#profileable-v-debuggable) (use **Profiler: run 'app' as debuggable (complete data)**) to record Java/Kotlin allocations.

![](/static/studio/images/profiler-java-kotlin-allocations-recording.png)
...
To force a garbage collection event while recording, click the garbage icon ![](/static/studio/images/profiler-garbage-collection.png).
...
![](/static/studio/images/profiler-java-kotlin-allocations-parsed.png)
...
![](/static/studio/images/profiler-jk-allocations-visualization.png)

  * Use the heap drop-down to filter to certain heaps. In addition to the [filters available when you capture a heap dump](/studio/profile/capture-heap-dump#heap-dump-overview), you can filter to classes in the JNI heap, the heap that shows where Java Native Interface (JNI) references are allocated and released.
...
![](/static/studio/images/profiler-jk-allocations-legend.png)
...
_Java Native Interface (JNI)_ is a framework that lets Java code and native code call each other. JNI references are managed manually by the native code, so it's possible for issues including the following to occur:
...
![](/static/studio/images/profiler-jk-allocations-jni-instances.png)

The **Allocation Call Stack** tab shows you where the JNI references are allocated and released in your code.

![](/static/studio/images/profiler-jk-allocations-jni-instance-details.png)
