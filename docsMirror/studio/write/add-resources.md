...
**Source:** [https://developer.android.com/studio/write/add-resources](https://developer.android.com/studio/write/add-resources)
...
In Jetpack Compose, you define your UI in Kotlin rather than XML layouts. However, you still use the `res/` directory to store app-level static assets like icons, fonts, and translations. To learn how to access these resources in your composable functions, see [Resources in Compose](/develop/ui/compose/resources).
...
Although the preceding page links describe workflows that are customized to each type of resource, you can add any XML resource file by following these steps:
  ...
  2. Select **File > New > Android resource file**. ![](/static/studio/images/write/new-resource_2-2_2x.png)
...
**Tip:** To open a simplified version of the **New Resource File** dialog that's specific to the resource type you want to add, right-click an existing resource directory within the **res** folder and select **New > type-name resource file**.

### Inline complex XML resources

Some complex resources require multiple XML resource files. For example, while Compose has robust [animation APIs](/develop/ui/compose/animation/introduction), you might need to use an animated vector drawable, which has a vector drawable object and an animation object and requires at least three XML files.
...
To add a new resource directory, follow these steps:
  ...
  2. Select **File > New > Android resource directory**. ![](/static/studio/images/write/new-resource-dir_2-2_2x.png)
