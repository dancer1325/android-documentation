# Get started with adaptive apps  |  Jetpack Compose  |  Android Developers

**Source:** [https://developer.android.com/develop/ui/compose/layouts/adaptive](https://developer.android.com/develop/ui/compose/layouts/adaptive)
...
Play ranks apps and games optimized for large screens higher than unoptimized apps. Play bases the ranking on the [large screen app quality guidelines](/docs/quality-guidelines/large-screen-app-quality). Higher ranking increases discoverability by enabling multi‑device users to see large screen–specific ratings and reviews on their phones.
...
![Google Play store listing for an app with a warning that says 'May not be optimized for your device'.](/static/develop/ui/compose/images/layouts/adaptive/build-adaptive-apps/app_with_Play_warning_message.png) **Figure 2.** Technical quality warning on app details page. 
...
[Window size classes](/develop/ui/compose/layouts/adaptive/window-size-classes) categorize app windows as _compact_ , _medium_ , or _expanded_ based on window width or height.

![Depiction of the compact, medium, and expanded width window size classes.](/static/develop/ui/compose/images/layouts/adaptive/window-size-classes/window_size_classes_width.png) **Figure 3.** Window size classes based on display width.  **Note:** Because vertical scrolling is commonplace, window width is typically the main consideration in determining window size class.

Compute your app's [`WindowSizeClass`](/reference/androidx/window/core/layout/WindowSizeClass) using the [`currentWindowAdaptiveInfo()`](/reference/kotlin/androidx/compose/material3/adaptive/package-summary#currentWindowAdaptiveInfo\(\)) top‑level function of the [Compose Material 3 Adaptive library](/reference/kotlin/androidx/compose/material3/adaptive/package-summary). The function returns an instance of [`WindowAdaptiveInfo`](/reference/kotlin/androidx/compose/material3/adaptive/WindowAdaptiveInfo), which contains [`windowSizeClass`](/reference/kotlin/androidx/compose/material3/adaptive/WindowAdaptiveInfo#windowSizeClass\(\)). Your app receives updates whenever the window size class changes:
...
An activity's layout is sometimes referred to as a _screen_. For example, your app might have a home screen, a list screen, and an item detail screen. The terminology implies that each activity fills the device screen.

However, on device screens that are large enough to support the expanded width window size class, multiple activity _screens_ can be onscreen at the same time. _Pane_ is a more precise term for content displays of individual activities.

Window size classes enable you to determine how many content panes to show in multi‑pane layouts, as specified in [Material Design](https://m3.material.io/foundations/layout/applying-layout/window-size-classes#70d1b184-a940-4169-a79a-b6643c470e73).

![Device screens split into panes: one pane in compact and medium window size classes, two panes in expanded window size class.](/static/develop/ui/compose/images/layouts/adaptive/build-adaptive-apps/panes_for_window_size_classes.png) **Figure 4.** Number of content panes per window size class. 
...
The [Compose Material 3 Adaptive library](/reference/kotlin/androidx/compose/material3/adaptive/package-summary) contains composables that manage window size classes, navigation components, multi‑pane layouts, and foldable postures and hinge location, for example:

  * [`NavigationSuiteScaffold`](/reference/kotlin/androidx/compose/material3/adaptive/navigationsuite/NavigationSuiteScaffold.composable): Automatically switches between navigation bar and navigation rail depending on app window size class and device posture.

  * [`ListDetailPaneScaffold`](/reference/kotlin/androidx/compose/material3/adaptive/layout/ListDetailPaneScaffold.composable): Implements the [list-detail canonical layout](/guide/topics/large-screens/large-screen-canonical-layouts#list-detail).
...
Adapts the layout to the app window size. Presents a list and the detail of a list item in side‑by‑side panes on the expanded window size class, but just the list or the detail on compact and medium window size classes.
  ...
  * [`SupportingPaneScaffold`](/reference/kotlin/androidx/compose/material3/adaptive/layout/SupportingPaneScaffold.composable): Implements the [supporting pane canonical layout](/guide/topics/large-screens/large-screen-canonical-layouts#supporting_pane).
...
By default, configuration changes recreate the app activity, and all activity state is lost. To maintain continuity, adaptive apps save state in the activity's [`onSaveInstanceState()`](/reference/kotlin/android/app/Activity#onsaveinstancestate_1) method or in a [`ViewModel`](/reference/kotlin/androidx/lifecycle/ViewModel).

#### Posture

Adaptive apps respond to changes in the posture of foldable devices. Postures include tabletop and book posture.

![A foldable phone in tabletop mode, with the bottom half of the screen resting on a surface and the top half upright.](/static/develop/ui/compose/images/layouts/adaptive/build-adaptive-apps/foldable_in_tabletop_posture.png) **Figure 7.** Foldable device in tabletop posture. 

The [`WindowInfoTracker`](/reference/kotlin/androidx/window/layout/WindowInfoTracker) interface in Jetpack WindowManager lets you to obtain a list of [`DisplayFeature`](/reference/kotlin/androidx/window/layout/DisplayFeature) objects for the device. Among the display features is [`FoldingFeature.State`](/reference/kotlin/androidx/window/layout/FoldingFeature.State), which indicates whether the device is fully or half open.

The Compose Material 3 Adaptive library provides the [`currentWindowAdaptiveInfo()`](/reference/kotlin/androidx/compose/material3/adaptive/package-summary#currentWindowAdaptiveInfo\(\)) top‑level function, which returns an instance of [`WindowAdaptiveInfo`](/reference/kotlin/androidx/compose/material3/adaptive/WindowAdaptiveInfo) containing [`windowPosture`](/reference/kotlin/androidx/compose/material3/adaptive/WindowAdaptiveInfo#windowPosture\(\)).
...
[Test different screen and window sizes](/training/testing/different-screens) and different device configurations. Use host‑side screenshots and Compose previews to check your app layouts. Run your app on Android Studio emulators and remote Android devices hosted in Google data centers.

### Large screen app quality guidelines

The [large screen app quality guidelines](/docs/quality-guidelines/large-screen-app-quality) help you verify that your adaptive app works well on tablets, foldables, and ChromeOS devices. The guidelines include tests that enable you to verify app functionality for critical user journeys. Although the guidelines focus on large screens, they're compatible with all screen sizes.

### Multiple configurations

The [`DeviceConfigurationOverride`](/reference/kotlin/androidx/compose/ui/test/DeviceConfigurationOverride) interface in Compose 1.7 and higher lets you to override various aspects of device configuration. The API simulates different device configurations in a localized way for whatever composable content you want to test. For example, you can test multiple, arbitrary UI sizes in a single run of your test suite on a single device or emulator.

With the [`DeviceConfigurationOverride.then()`](/reference/kotlin/androidx/compose/ui/test/DeviceConfigurationOverride#\(androidx.compose.ui.test.DeviceConfigurationOverride\).then\(androidx.compose.ui.test.DeviceConfigurationOverride\)) extension function, you can test multiple configuration parameters, such as font size, locale, theme, and layout size, all at the same time.
...
For more information, see [Compose Preview Screenshot Testing](/studio/preview/compose-screenshot-testing).

### Compose previews

Compose previews let you check your app's UI in the design view of Android Studio. Previews use annotations, such as [`@PreviewScreenSizes`](/reference/kotlin/androidx/compose/ui/tooling/preview/PreviewScreenSizes), [`@PreviewFontScale`](/reference/kotlin/androidx/compose/ui/tooling/preview/PreviewFontScale), and [`@PreviewLightDark`](/reference/kotlin/androidx/compose/ui/tooling/preview/PreviewLightDark) to let you see composable content in various configurations. You can even interact with the previews.

Android Studio also highlights common usability issues in previews, such as buttons or text fields that are too wide.

For more information, see [Preview your UI with composable previews](/develop/ui/compose/tooling/previews).
...
Remote device streaming is integrated into Android Studio. For more information, see [Android Device Streaming, powered by Firebase](/studio/run/android-device-streaming).
