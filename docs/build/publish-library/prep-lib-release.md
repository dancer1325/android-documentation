# Prepare your library for release  |  Android Studio  |  Android Developers

**Source:** [https://developer.android.com/build/publish-library/prep-lib-release](https://developer.android.com/build/publish-library/prep-lib-release)
...
Starting with AGP 7.0, you can set the [namespace](/reference/tools/gradle-api/7.0/com/android/build/api/dsl/CommonExtension#namespace:kotlin.String) in the app’s `build.gradle` file, as shown in the following code example:
...
The namespace is a developer-facing property of the library. It is not related to the application identity, which is set using the [`applicationId`](/studio/build/configure-app-module#set_the_application_id) property.

In previous versions of AGP, both the `applicationId` property (for an app) and the `namespace` property (for a library) could be set using the manifest's [`package`](/guide/topics/manifest/manifest-element#package) attribute, which led to confusion.
...
Choosing a [`minSdkVersion`](/guide/topics/manifest/uses-sdk-element#min) for your library is an important aspect of publishing your library. The `minSdkVersion` should reflect the minimum version of Android that your code can support.
...
Starting with version 4.1, AGP supports [`minCompileSdk`](/reference/tools/gradle-api/7.1/com/android/build/api/variant/AarMetadata#mincompilesdk). This indicates the minimum [`compileSdk`](/reference/tools/gradle-api/7.1/com/android/build/api/dsl/CommonExtension#compileSdk:kotlin.Int) that consuming projects can use. If your library contains manifest entries or resources that make use of newer platform attributes, you need to set this value.
...
For more information, refer to Gradle’s documentation about [using test fixtures](https://docs.gradle.org/current/userguide/java_testing.html#sec:java_test_fixtures).
