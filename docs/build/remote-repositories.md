# Manage remote repositories  |  Android Studio  |  Android Developers

**Source:** [https://developer.android.com/build/remote-repositories](https://developer.android.com/build/remote-repositories)
...
**Warning:** The JCenter repository, which was included by default in the past, became read-only on March 31st, 2021. For more information, see [JCenter service update](/studio/build/jcenter-migration).
...
Then add the desired library to your module's `dependencies` block. For example,the [appcompat library](/jetpack/androidx/releases/appcompat) looks like this:
...
_maven.google.com/ group_path/group-index.xml_
...
_maven.google.com/ group_path/library/version /library-version.ext_
...
For libraries not available from the Google Maven repository (usually older library versions), you must download the offline **Google Repository** package from the [SDK Manager](/studio/intro/update#sdk-manager).
