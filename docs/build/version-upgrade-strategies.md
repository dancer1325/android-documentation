# Upgrade dependency versions  |  Android Studio  |  Android Developers

**Source:** [https://developer.android.com/build/version-upgrade-strategies](https://developer.android.com/build/version-upgrade-strategies)
...
**Note:** Good [app tests](/training/testing) are key to ensuring upgrades don't break your build or app in unexpected ways.
...
| How often do you release your application or library?Shorter development and release cycles
  ...
  * If you use plugins or tools that help automate dependency upgrades, be sure to analyze the results to check for risks.
...
Upgrading some types of dependencies might have a cascading effect, requiring other types of dependencies to be upgraded. We discuss relationships between build elements in [Tool and library interdependencies](/build/tool-and-library-dependencies).

![Build dependencies and their relationships](/static/images/build/build-dependencies.svg) **Figure 1.** Build relationships.

When upgrading each type of component, consider how the upgrade impacts other components in the build.

### [Android Gradle Plugin (AGP)](/build/releases/gradle-plugin)
...
### [Kotlin compiler, language and runtime](https://kotlinlang.org/)

| Check the [Kotlin release notes](https://kotlinlang.org/docs/releases.html#release-details) for known issues and incompatibilities.If you use Jetpack Compose:

  * If the new Kotlin version is lower than 2.0.0:
    * Locate the [compatible Compose compiler plugin](/jetpack/androidx/releases/compose-kotlin#pre-release_kotlin_compatibility).
    * Upgrade `kotlinCompilerExtensionVersion` in the `build.gradle.kts` of any modules where it appears.
  * If the new Kotin version is 2.0.0 or higher:
    * Set up the [Compose Compiler Gradle Plugin](/develop/ui/compose/compiler) with the same version as the Kotlin compiler.
...
### [Kotlin Compiler Plugins](https://kotlinlang.org/docs/all-open-plugin.html)
...
### [Gradle](/build/gradle-build-overview)
...
| Sometimes new versions of Gradle deprecate existing APIs, removing those APIs in a future release. If you develop a Gradle plugin, upgrade your plugin as soon as possible, especially if that plugin is public.Some Gradle upgrades require locating new versions of plugins that you use. Note that these plugins can lag in their development as they upgrade the plugins to match the latest Gradle plugin APIs.To upgrade Gradle:
  ...
  * Read the [release notes](https://gradle.org/releases/) for the version you want to use.
...
The first step in your upgrade analysis is to determine how your dependencies change. Take advantage of version control (VCS, such as Git) and the [Dependency Guard](https://github.com/dropbox/dependency-guard) plugin to quickly see changes. Your goal is to create a _before_ and _after_ snapshot and compare them.
...
to your `build.gradle.kts` in each module.
    ...
    * Or, if you have many existing warnings, create an [Android lint baseline](/studio/write/lint#snapshot). Over time, be sure to clean up existing Warnings (or suppress those you don't care about)
...
Run `./gradlew lint` to see new lint warnings or errors. Address any important issues and then update your warning baseline by running `./gradlew lint -Dlint.baselines.continue=true`. If you have used other tools to capture warning baselines, address new warnings and update their baselines as well.
...
[Android lint](/studio/write/lint) can catch many issues in your application, including some that are the result of changing versions of dependencies or the Android SDK. For example, if you upgrade your [`compileSdk`](/build#android_sdk_settings) and use its new APIs, lint reports those that aren't available in prior SDK versions.
...
If you use [Continuous Integration](/training/testing/continuous-integration) (CI), run `gradlew build` or `gradlew lint` during your CI builds (or at least on your nightly builds) to catch these types of errors.
...
After determining upgrade risks, decide how you want to mitigate them:
  ...
  * Mitigate remaining risks 
    ...
    * [Test your app](/training/testing) to check for unexpected changes. Add new tests where needed to build confidence in the upgrade.
