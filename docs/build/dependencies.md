...
**Source:** [https://developer.android.com/build/dependencies](https://developer.android.com/build/dependencies)
...
Some libraries are available in a published Bill of Materials (BOM) that groups families of libraries and their versions. You can include a BOM in your version catalog and build files, and let it manage those versions for you. See [Using the Bill of Materials](/jetpack/compose/bom) for details.
...
For more information about using annotation processors, see [Add annotation processors](/build/annotation-processors).  
...
All of the preceding configurations apply dependencies to all build variants. If you instead want to declare a dependency for only a specific [build variant](/studio/build/build-variants) source set or for a [testing source set](/studio/test#sourcesets), you must capitalize the configuration name and prefix it with the name of the build variant or testing source set.
...
Android Studio shows lint warnings in the version catalog file and the **Project Structure Dialog** for public SDKs in the [Google Play SDK Index](/distribute/sdk-index) when the following issues apply:
