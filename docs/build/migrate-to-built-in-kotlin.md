...
**Source:** [https://developer.android.com/build/migrate-to-built-in-kotlin](https://developer.android.com/build/migrate-to-built-in-kotlin)
...
**Note:** You can also enable built-in Kotlin for one module at a time.
...
**Note:** If you're not yet ready to migrate your project, you can also opt out of built-in Kotlin.
...
The `org.jetbrains.kotlin.kapt` (or `kotlin-kapt`) plugin is incompatible with built-in Kotlin. If you use `kapt`, we recommend that you [migrate your project to KSP](/build/migrate-to-ksp).
...
If you want to add a Kotlin source directory to a specific variant or if the directory is generated by a task, you can use the [`addStaticSourceDirectory`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/SourceDirectories#addStaticSourceDirectory\(kotlin.String\)) or [`addGeneratedSourceDirectory`](/reference/tools/gradle-api/9.0/com/android/build/api/variant/SourceDirectories#addGeneratedSourceDirectory\(org.gradle.api.tasks.TaskProvider,kotlin.Function1\)) methods in the [variant API](/build/extend-agp#variant-api-artifacts-tasks):
...
**Note:** You also need to set `android.newDsl=false` to opt out of the [new DSL](/r/tools/new-dsl) because the `kotlin-android` plugin is not compatible with it.
