# Reduce game size  |  Android game development  |  Android Developers

**Source:** [https://developer.android.com/games/optimize/game-size](https://developer.android.com/games/optimize/game-size)
...
To reduce the size of a game, perform the following steps:
  ...
  1. Use optimized delivery methods such as App Bundles and Play Asset Delivery.
  2. Determine the baseline size of the game and understand its structure.
...
Consider the following delivery methods:
  ...
  * While legacy projects typically output an APK for publishing, games publishing on Google Play should use an [Android App Bundle](/guide/app-bundle). App bundles deliver optimized APKs tailored to each user's device configuration. On average, app bundles reduce app size by 20%.

  * To serve game assets via Google Play and exceed the 200MB download size limit for app bundles up to multiple GBs, use [Play Asset Delivery (PAD)](/guide/app-bundle/asset-delivery). No external CDN is required. PAD supports three distinct delivery methods, each of which can be used in a single game:
...
PAD requires that games are packaged using the App Bundle format. PAD can also reduce the amount of data used by a developer's CDN (if one is still needed).
...
This section describes how to reduce the size of files in an APK. You can use the [App Bundle Explorer](https://play.google.com/console/about/app-bundle-explorer/) to download device-specific APKs.
...
To reduce the size of files packaged directly into the APK, perform the following steps:

  1. Use the Android Studio [APK Analyzer](/studio/debug/apk-analyzer). In Android Studio, select **File > Profile or debug APK** and select your APK.
  2. Select the **Assets** folder. For each file, the raw file size and the percentage of the total download size are listed.
  3. Review the **Assets** folder and determine where the bulk of the game's data is located. Determine whether there are any assets that take up an excessive amount of space (that is, greater than 1% of the total). In particular, look for the following:
  ...
  4. If multiple [Application Binary Interfaces](/ndk/guides/abis) (ABIs) are included in the APK, move to [App Bundles](/guide/app-bundle) or build multiple APKs.

  5. Determine the size of the binary shared object (.so) files. To inspect binary files to see whether there are things that might not need to be included, such as log files, you can use the [Bloaty McBloatface tool](https://github.com/google/bloaty). For more information about how to optimize the binary files, refer to [compile flags and options](/studio/build/shrink-code#optimization).
...
To inspect the textures in the game, use [Android GPU Inspector](/agi) (AGI), [RenderDoc](https://renderdoc.org/), or, for Qualcomm Snapdragon GPUs only, [Snapdragon Profiler](https://developer.qualcomm.com/software/snapdragon-profiler).
...
Look for the following:
  ...
  * Discard the top mipmap level when loading the textures on lower end devices to save memory. Unity's [Texture Streaming system](https://docs.unity3d.com/Manual/TextureStreaming.html) can do this.
...
Follow these recommendations for an APK's assets:
  ...
  * Reduce texture resolution: A texture much larger than the number of pixels that ultimately render on the screen is an inefficient use of space and GPU resources. To change a texture and see what it looks like in a frame without the need to rebuild the game, use [AGI](/agi).
