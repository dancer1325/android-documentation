# Additional rule types  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/app-optimization/additional-rule-types](https://developer.android.com/topic/performance/app-optimization/additional-rule-types)
...
**Note:** For more rule types, see [Add keep rules](/topic/performance/app-optimization/add-keep-rules), [Troubleshooting rules](/topic/performance/app-optimization/troubleshooting-rules), and [Global options](/topic/performance/app-optimization/global-options).

## Assumptions

These rules tell R8 that it can make specific assumptions about specific code behavior at runtime.

**Warning:** Assumptions instruct R8 to force further optimizations on your code, beyond what the code is analyzed to do. Use assumptions with caution, because forcing optimizations can break your app. Make sure to [test the optimization](/topic/performance/app-optimization/test-the-optimization) thoroughly when using these rules.
...
The syntax for `-assumevalues` is similar to the syntax for keeping a [`member_specification`](/topic/performance/app-optimization/add-keep-rules#member-spec), but additionally includes a `return clause` as follows:
...
The syntax for `-assumenosideeffects` is similar to the syntax for keeping a [`member_specification`](/topic/performance/app-optimization/add-keep-rules#member-spec).
