# Find the unresponsive thread  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/anrs/find-unresponsive-thread](https://developer.android.com/topic/performance/anrs/find-unresponsive-thread)
...
Content provider not responding | Either: 

  * Binder thread if ANR is caused by a slow content provider query.
  * Main thread if ANR is caused by a long app startup.
...
Although most binder calls are quick, the long tail can be very slow. This is more likely to happen if the device is loaded or the binder reply thread is slow, such as from lock contention, many incoming binder calls, or [hardware abstraction layer (HAL)](/guide/platform#hal) timeout.
