# Case study: how the Gmail Wear OS team improved their app startup by 50%  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/appstartup/case-study-gmail-wear](https://developer.android.com/topic/performance/appstartup/case-study-gmail-wear)
...
![Android App Startups and main thread rows pinned.](/static/topic/performance/images/gmail-wear-main-thread-pinned.png) **Figure 3.** Pin the main thread rows underneath the Android App Startups custom metric to aid analysis.
...
![Main thread highlighted with total time in different states in Thread States panel.](/static/topic/performance/images/gmail-wear-runnable-state.png) **Figure 4.** Assess the relative time in `Runnable` to `Running` states to get an initial sense of how much CPU contention there is.
...
**Tip:** To reduce the time spent in the `OpenDexFilesFromOat*` state, use [R8 minification](/build/shrink-code) to reduce the amount of code that needs to be converted to DEX files.
...
**Tip:** The [CPU profiler](/studio/profile/jank-detection) in Android Studio can show you your app's activity with respect to the device's Vsync rate.
...
![A line connects the binder call and reply.](/static/topic/performance/images/gmail-wear-binder-transaction.png) **Figure 5.** Identify the binder transactions happening during app startup and assess whether you can defer them.
...
As a result of this analysis, the Gmail Wear OS team made the following improvements:
  ...
  * From looking at time spent in `OpenDexFilesFromOat*` and JIT activity, they enabled [R8](/build/shrink-code#shrink-code) rewriting of [Baseline Profiles](/topic/performance/baselineprofiles/create-baselineprofile). This improved app startup latency by 20%.

**Tip:** To enable R8 rewriting of Baseline Profiles, make sure you're using AGP 8.2 or higher. [See the functionalities that come with each version of AGP](/topic/performance/baselineprofiles/overview#recommended-versions).
