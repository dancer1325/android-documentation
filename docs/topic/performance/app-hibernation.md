# App hibernation  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/app-hibernation](https://developer.android.com/topic/performance/app-hibernation)
...
When an [activity](/guide/components/activities/intro-activities) in your app is resumed, the system considers this event to be a user interaction. Therefore, the system extends the amount of time before your app enters hibernation.

On Android 11 and higher, the following behaviors are also considered to be user interactions:

  * The user interacts with a [widget](/guide/topics/appwidgets/overview).
  * The user interacts with a notification, except for dismissing the [notification](/guide/topics/ui/notifiers/notifications).
...
Work profile apps
    Any app that a user installs on a [work profile](https://support.google.com/work/android/answer/6191949). Note that if the same app also resides on a personal profile, only the work profile app is exempt.
Device policy controllers
    Apps that control [local device policies](/work/dpc/build-dpc) and system applications on devices.
...
**Note:** If your app is a [device policy controller](/work/dpc/build-dpc) that implements [`DeviceAdminService`](/reference/android/app/admin/DeviceAdminService), runs in the [device owner or profile owner mode of operation](https://developers.google.com/android/work/play/emm-api/prov-devices#modes_of_operation), or is a [carrier-privileged app](https://source.android.com/devices/tech/config/carrier), the system never places your app into a hibernation state.
...
To check whether the user has already disabled hibernation for your app, use the [`getUnusedAppRestrictionsStatus()`](/reference/androidx/core/content/PackageManagerCompat#getUnusedAppRestrictionsStatus\(android.content.Context\)) API.
...
If the user hasn't already disabled hibernation for your app, you can send a request to the user. To do so, complete these steps:
  ...
  2. Invoke the [`createManageUnusedAppRestrictionsIntent()`](/reference/androidx/core/content/IntentCompat#createManageUnusedAppRestrictionsIntent\(android.content.Context,%20java.lang.String\)) API, as shown in the API code example. This API creates an intent that loads the **App info** screen in Settings. From here, the user can turn off hibernation for your app. 
...
**Table 2.** Option that disables hibernation for your app Characteristics of device | Page where the option appears | Name of the option to turn off  
---|---|---  
...
### Kotlin
    ...
    fun onResult(appRestrictionsStatus: Int) {
      when (appRestrictionsStatus) {
        ...
        [API_30_BACKPORT](/reference/androidx/core/content/UnusedAppRestrictionsConstants#API_30_BACKPORT), [API_30](/reference/androidx/core/content/UnusedAppRestrictionsConstants#API_30), [API_31](/reference/androidx/core/content/UnusedAppRestrictionsConstants#API_31) -> handleRestrictions(appRestrictionsStatus)
      }
    }
...
To test how your app behaves after the system places your app in a hibernation state, complete the following steps:
  ...
  1. _(Android 12 and higher only)_ Enable the hibernation behavior on your device:
...
When the broadcasts are finished, this command returns the message: `All broadcast queues are idle!`
  ...
  6. _(Android 12 and higher only)_ Confirm that the app is hibernated, using one of the following methods:
