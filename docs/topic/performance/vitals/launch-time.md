...
**Source:** [https://developer.android.com/topic/performance/vitals/launch-time](https://developer.android.com/topic/performance/vitals/launch-time)
...
Two important metrics for determining app startup are time to initial display (TTID) and time to fully drawn (TTFD). TTID is the time it takes to display the first frame, and TTFD is the time it takes for the app to become fully interactive. Both are equally important, because TTID lets the user know that the app is loading, and TTFD is when the app is actually useable. If either of these is too long, the user might exit your app before it even fully loads.
...
![](/static/topic/performance/images/cold-launch.png) **Figure 1.** A visual representation of the important parts of a cold app launch.
...
If you override [`Application.onCreate()`](/reference/android/app/Application#onCreate\(\)) in your own app, the system invokes the `onCreate()` method on your app object. Afterwards, the app spawns the main thread, also known as the _UI thread_ , and tasks it with creating your main activity.

From this point, system- and app-level processes proceed in accordance with the [app lifecycle stages](/guide/topics/processes/process-lifecycle).
...
However, if some memory is purged in response to memory trimming events, such as [`onTrimMemory()`](/reference/android/content/ComponentCallbacks2#onTrimMemory\(int\)), then these objects need to be recreated in response to the hot start event.
...
![](/static/topic/performance/vitals/images/startup-modes-r1.png) **Figure 2.** A diagram with the various startup states and their respective processes, each state beginning from the first frame drawn.
...
To debug app startup issues, it's helpful to determine what exactly is included in the app startup phase. To identify the entire app startup phase in [Perfetto](https://perfetto.dev/docs/), follow these steps:
  ...
  1. In Perfetto, find the row with the Android App Startups derived metric. If you don't see it, try capturing a trace using the [on-device system tracing app](/topic/performance/tracing/on-device).

![](/static/topic/performance/images/app-startup-perfetto.png) **Figure 3.** The Android App Startups derived metric slice in Perfetto.
  2. Click the associated slice and press `m` to select the slice. Brackets appear around the slice and denote how long it took. The duration is also shown in the **Current selection** tab.
  ...
  5. Zoom into the main thread, usually at the top, by pressing `w` (press `s, a, d` to zoom out, move left, and move right, respectively).
...
Android uses the time to initial display (TTID) and time to full display (TTFD) metrics to optimize cold and warm app startups. Android Runtime (ART) uses the data from these metrics to efficiently precompile code for optimization of future startups.
...
To find TTID, search in the [Logcat command-line tool](/tools/logcat) for an output line containing a value called `Displayed`. This value is the TTID and looks similar to the following example, in which TTID is 3s534ms:
...
![](/static/topic/performance/images/displayed-logcat.png) **Figure 5.** Disabled filters and the `Displayed` value in logcat.
...
We recommend using Logcat in Android Studio, but If you're not using Android Studio, you can also measure TTID by running your app with the [`adb` shell activity manager command](/tools/adb#am). Here's an example:
...
The `-c` and `-a` arguments are optional and let you specify [`<category>`](/guide/topics/manifest/category-element) and [`<action>`](/guide/topics/manifest/action-element).
...
The Logcat output sometimes includes a `total` time, as discussed in Time to initial display.
...
You can also gain insight into potential bottlenecks through inline tracing inside of your apps' and activities' `onCreate()` methods. To learn about inline tracing, see the documentation for the [`Trace`](/reference/android/os/Trace) functions and the [overview of system tracing](/topic/performance/tracing).
...
Running the CPU Profiler reveals that the [`callApplicationOnCreate()`](/reference/android/content/ComponentCallbacks2#onTrimMemory\(int\)) method eventually calls your `com.example.customApplication.onCreate` method. If the tool shows that these methods are taking a long time to finish executing, explore further to see what work is occurring there.

##### Inline tracing

Use inline tracing to investigate likely culprits, including the following:

  * Your app's initial [`onCreate()`](/reference/android/app/Application#onCreate\(\)) function.
...
Also, consider using a dependency injection framework like [Hilt](/training/dependency-injection/hilt-android) that creates objects and dependencies when they are injected for the first time.

If your app uses content providers to initialize app components at startup, consider using the [App Startup library](/topic/libraries/app-startup) instead.
...
When using the CPU Profiler, pay attention to your app's [`Application`](/reference/android/app/Application) subclass constructors and `com.example.customApplication.onCreate()` methods.
...
There are many potential bottlenecks, but two common problems and remedies are as follows:
  ...
  * The larger your view hierarchy, the more time the app takes to inflate it. Two steps you can take to address this issue are the following: 
    ...
    * Don't inflate parts of the UI that don't need to be visible during launch. Instead, use a [`ViewStub`](/reference/android/view/ViewStub) object as a placeholder for sub-hierarchies that the app can inflate at a more appropriate time.
...
See the [Splash screen migration guide](/guide/topics/ui/splash-screen/migrate) for details.
