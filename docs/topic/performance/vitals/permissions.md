# Permission Denials  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/vitals/permissions](https://developer.android.com/topic/performance/vitals/permissions)
...
A _daily permission session_ refers to a day during which your app requested at least one permission from a user. When a given user has to make multiple decisions for the same permission, only the final decision at the end of a session is recorded.
...
Consider starting by requesting your permission in-context: Providing an explanation for less intuitive permissions helps to improve user comprehension of the permission. The [`shouldShowRequestPermissionRationale()`](/reference/androidx/core/app/ActivityCompat#shouldShowRequestPermissionRationale\(android.app.Activity,%20java.lang.String\)) utility method returns true if the user has previously denied the request. Your app can use this method to determine when to show the explanation.

You can find more details about how to surface explanation strings in [Request App Permissions](/training/permissions/requesting).

[ Previous arrow_back  Excessive Battery Usage  ](/topic/performance/vitals/excessive-battery-usage)

[ Next App Startup  arrow_forward  ](/topic/performance/vitals/launch-time)
