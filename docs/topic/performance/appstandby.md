# App Standby Buckets  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/topic/performance/appstandby](https://developer.android.com/topic/performance/appstandby)
...
If an app is in the working set, the system imposes mild restrictions on its ability to run jobs and trigger alarms. For details, see [Power management resource limits](/topic/performance/power/power-details#app-stdby-bucket).
...
If an app is in the frequent bucket, the system imposes stronger restrictions on its ability to run jobs and trigger alarms. For details, see [Power management resource limits](/topic/performance/power/power-details#app-stdby-bucket).
...
If an app is in the rare bucket, the system imposes strict restrictions on its ability to run jobs and trigger alarms. The system also limits the app's ability to connect to the internet. For details, see [Power management resource limits](/topic/performance/power/power-details#app-stdby-bucket).
...
If the system places your app in the restricted bucket, the following restrictions apply:
  ...
  * Your app can run fewer [expedited jobs](/topic/libraries/architecture/workmanager/how-to/define-work#expedited), compared to when the system places your app in a less restrictive bucket.
  * Your app can invoke one alarm per day. This alarm can be either an [exact alarm](/training/scheduling/alarms#exact) or an [inexact alarm](/training/scheduling/alarms#inexact).
...
The following types of apps are exempt from entering the restricted bucket and bypass the inactivity trigger, even on Android 12 and higher:
  ...
  * [Companion device](/guide/topics/connectivity/companion-device-pairing) apps
  ...
  * [Persistent](/guide/topics/manifest/application-element#persistent) apps
  * VPN apps
  * Apps that have the [`ROLE_DIALER`](/reference/android/app/role/RoleManager#ROLE_DIALER) role
  * Apps that the user has explicitly designated to provide "unrestricted" functionality in system settings
  * Apps with active [widgets](/guide/topics/appwidgets/overview)
...
To check which bucket your app is assigned to, do one of the following:
  ...
  * Call [`getAppStandbyBucket()`](/reference/android/app/usage/UsageStatsManager#getAppStandbyBucket\(\)).
...
If your app is following best practices for Doze and app standby, the later power management features aren't difficult. However, some app behaviors which previously worked well might cause problems.
  ...
  * If the users can't interact with app notifications, users are unable to trigger the app's promotion to the active bucket. In this case, consider redesigning some notifications that let users interact. For some guidelines, see the Material Design [Notifications design patterns](https://material.io/guidelines/patterns/notifications.html).

  * If the app doesn't show a notification upon receiving a [high-priority Firebase Cloud Messaging (FCM) message](https://firebase.google.com/docs/cloud-messaging/android/message-priority#using_high_priority_messages_for_android), the user can't interact with the app and thus promote it to the active bucket. In fact, the only intended use for high priority FCM messages is to push a notification to the user, so this situation must not occur. On 12L (API level 32) and lower, if you inappropriately mark an FCM message as high priority when it doesn't trigger user interaction, it can cause future messages to be deprioritized.
