# What great technical quality looks like  |  App quality  |  Android Developers

**Source:** [https://developer.android.com/quality/technical](https://developer.android.com/quality/technical)
...
If it makes sense for your app or game to support multiple form factors, ensure [continuity](/develop/ui/compose/layouts/adaptive/foldables/learn-about-foldables#app_continuity) across form factors for a seamless user experience—for example, by synchronizing files and settings between devices, or saving their progress.
...
Following best practices, such as [opting into GWP-ASan](/ndk/guides/gwp-asan), programming in null-safe languages such as [Kotlin](/kotlin), and using [background work APIs](/guide/background), can reduce stability issues and also make debugging easier when they do occur.
...
Providing a [baseline profile](/topic/performance/baselineprofiles/overview) can improve rendering performance and startup time. Consider using the [JankStats](/topic/performance/jankstats) library to track and analyze performance issues. Review the [best practices](/topic/performance/rendering) for rendering.
...
Follow best practices, such as using the [Android Dynamic Performance Framework](/games/optimize/adpf), [Game Mode API](/games/gamemode/about-API-and-interventions) and [frame pacing](/games/sdk/frame-pacing), to optimize visual smoothness and stability. Use [Android Performance Tuner](/games/sdk/performance-tuner) to tune quality levels appropriately for the devices you support. Make considered choices about graphics libraries and asset formats. For example, using [Vulkan](/ndk/guides/graphics/getting-started) as a graphics API and [ASTC for assets](/guide/playcore/asset-delivery/texture-compression) can significantly improve your rendering performance.
...
Thoughtful and appropriate usage of limited or costly resources like battery life and network bandwidth will make your app accessible to more users, increase session length, and improve user retention. Games should reduce frame rates and display refresh rates where appropriate, for example when rendering menus and loading screens. Using the [Game Mode API](/games/gamemode/about-API-and-interventions) can help users make tradeoffs between performance and battery life, and can lead to [longer playtime](/stories/games/gameloft-gamemode).

### Google Play guidelines

If you distribute on Google Play, follow these additional battery and network usage guidelines.

**Tools to monitor and optimize battery and network usage** Use Android vitals in Play Console or the reporting API to monitor the battery and network metrics that matter most to users and Google Play.
...
Follow recommended best practices to minimize the size of your [game](/games/optimize/game-size) or [app](/topic/performance/reduce-apk-size).

### Google Play guidelines

If you distribute on Google Play, follow these additional app size guidelines.

**Tools to monitor and optimize app size** Use Android vitals in [Play Console](https://play.google.com/console/about/vitals/) to monitor your app size. Android vitals allows you to compare your app size to your peers, and helps you understand how many devices in your user base are running low on storage. Keep in mind that Google Play proactively helps users free up device storage by suggesting apps to uninstall, and will prioritize app size when formulating these recommendations.

If you distribute on Google Play, use the [app bundle format](/guide/app-bundle) to ensure that each user downloads only the code and resources necessary to run your app or game. Larger apps and games can further benefit from [Play Feature delivery](/guide/playcore/feature-delivery) and [Play Asset delivery](/guide/playcore/asset-delivery), where specific parts of your code or assets can be downloaded conditionally or on demand.
...
**Tools to increase app freshness** Not all users enable background updates. Adopting features like [in-app updates](/guide/playcore/in-app-updates) can increase the number of active users on the latest version of your app or game.
...
To minimize the risk of introducing new issues in a release, take a phased approach to [testing](/training/testing) and rollout, and monitor your metrics frequently during any changes. You can also make it easier to mitigate emerging issues by decoupling binary releases from feature releases with remote configuration SDKs, such as [Firebase Remote Config](https://firebase.google.com/docs/remote-config).
