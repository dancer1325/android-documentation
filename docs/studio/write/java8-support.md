...
**Source:** [https://developer.android.com/studio/write/java8-support](https://developer.android.com/studio/write/java8-support)
...
In some cases, your module might not be using the `invoke` or `invokeExact` methods even when they're included in a library dependency. To keep using that library with `minSdkVersion 25` or lower, [enable code shrinking](/studio/build/shrink-code#shrink-code) to remove unused methods. If that doesn't work, consider using an alternative library that doesn't use the unsupported methods.
